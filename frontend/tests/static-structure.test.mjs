...
function _readDir(dir) {
  ...
  for (const entry of _readdirSync(dir)) {
    const full = _join(dir, entry);
    if (_statSync(full).isDirectory()) result += _readDir(full);
    else if (entry.endsWith(".ts") || entry.endsWith(".tsx")) result += readFileSync(full, "utf8") + "\n";
  }
  return result;
}
const allSourceCached = _readDir(_fileURLToPath(new URL("../app/", import.meta.url)));
...
test("[静态] page.tsx 包含项目列表展开/收起控制逻辑", () => {
  ...
  assert.match(source, /setProjectListExpanded\(/);
...
test("[静态] 创建项目表单为单列布局且必填项用星号标注", () => {
  const pageSourceText = allSourceCached;
...
test("[静态] 人工确认表单包含 M5 案例选择且与尾页打印版同一行", () => {
  const source = allSourceCached;
  assert.match(source, /确认 M5 案例[\s\S]*添加尾页打印版/);
  assert.doesNotMatch(source, /确认 M3 模块/);
